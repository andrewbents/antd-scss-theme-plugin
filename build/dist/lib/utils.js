 ...
 */
const compileThemeVariables = exports.compileThemeVariables = (themeScssPath, modifyVars) => {
  ...
  return extractLessVariables(themeEntryPath, Object.assign(variableOverrides, modifyVars)).then(variables => Object.entries(variables).map(([name, value]) => `$${name}: ${value};\n`).join(''));
};